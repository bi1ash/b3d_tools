...
class empty_to_collection(bpy.types.Operator):
    """Converts selected empties to Collections"""
    bl_idname = "object.convert_empty"
...
def menu_func_outliner_object(self, context):
    if context.object: # Only show if an object is selected
       ...
       layout.operator(empty_to_collection.bl_idname)


def register():
    bpy.utils.register_class(empty_to_collection)
    bpy.types.OUTLINER_MT_object.append(menu_func_outliner_object)


def unregister():
    bpy.utils.unregister_class(empty_to_collection)
